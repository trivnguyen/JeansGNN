...
def jax_cumtrapz_init(y, x, initial=0):
    """
    Cumulative trapezoidal integration of y with respect to x with initial value.
    """
    dx = jnp.diff(x)
    res = jnp.cumsum((y[1:] + y[:-1]) / 2 * dx)
...
def calc_sigma2p_Sigma(R, r, sigma2_nu, beta):
    ...
    rminR2 = r**2 - R**2
    rminR2_abs = jnp.abs(rminR2)   # necessary for numpyro to work. DO NOT REMOVE
    beta = beta[None, :]

    inte = (1 - beta * R**2 / r**2) * sigma2_nu * r
    inte = jnp.where(rminR2 > 0.,  inte / jnp.sqrt(rminR2_abs), 0.)
